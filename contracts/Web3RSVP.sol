...
contract Web3RSVP {
    ...
    function returnClaim(bytes32 eventId, address attendee) public {
        ...
        // if this succeeds, add the user from the array of claimed RSVPs
        if (sent) {
    ...
    function confirmAttendee(bytes32 eventId, address attendee) public {
        ...
        require(rsvpConfirm, "NO RSVP TO CONFIRM");
        returnClaim(eventId, attendee);
    }
    ...
    function confirmAllAttendees(bytes32 eventId) external {
        ...
        for (uint8 i = 0; i < myEvent.confirmedRSVPs.length; i++) {
            returnClaim(eventId, myEvent.confirmedRSVPs[i]);
        }
    }
